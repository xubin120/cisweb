		...
		<div class="admin-content">
			...
			<div class="am-cf am-padding">
				<div class="am-fl am-cf">
					<strong class="am-text-primary am-text-lg">店务管理</strong>
				</div>
			</div>
				...
				<div class="am-u-md-6 am-cf">
					<div class="am-btn-toolbar am-fl am-cf">
						<div class="am-btn-group am-btn-group-xs">
							<button type="button" class="am-btn am-btn-default btn-click" onclick="location.href='preAddShop'">
					...
					<table class="am-table am-table-striped am-table-hover table-main">
						<thead>
							<tr>
								...
								<th width="140px">到店时间</th>
								<th width="90px">客户姓名</th>
								<th width="90px">美容师</th>
								<th >活动名称</th>
								<th width="120px">方案名称</th>
								<th width="120px">项目名称</th>
								<th width="100px">房间</th>
								<th width="80px">持续时间</th>
								<th width="100px">操作</th>
							</tr>
						</thead>
						<tbody>
							<s:iterator var="shop" value="shopList"> 
							<tr>
								...
								<td><s:property value="employeeVO.name"/></td>
								<td><s:property value="activityVO.name"/></td>
								<td><s:property value="planVO.name"/></td>
								<td><s:property value="itemVO.name"/></td>
								<td><s:property value="room"/></td>
								<td><s:property value="hours"/></td>
								<td>
									<div class="am-btn-toolbar">
										<div class="am-btn-group am-btn-group-xs">
											<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary btn-click" onclick="location.href='<s:url action="preEditShop"><s:param name="shopId" value="id" /></s:url>'">
						...
						<div class="am-fr">
							<ul class="am-pagination">
								<li class="<c:if test="${pi.hasPreviousPage=='false'}">am-disabled</c:if>"> 
									<a href="findAllShop?currentPage=${pi.firstPage}">首页</a>
								</li>
								
								<c:forEach items="${pi.navigatepageNums}" var="pageNums">
									<li class="" value="${pageNums}" id="pageNum">
										<a href="findAllShop?currentPage=${pageNums}">${pageNums}</a>
									</li>
								</c:forEach>
								
								<li class="<c:if test="${pi.hasNextPage=='false'}">am-disabled</c:if>">
									<a href="findAllShop?currentPage=${pi.lastPage}">尾页</a>
								</li>
							</ul>
						</div>
					</div>
